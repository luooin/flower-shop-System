layui.use(['element', 'jquery', 'layer', 'laytpl', 'laypage', 'form'], function () {
    ...
    //订单信息填写弹窗,并提交订单
    /**
     * 参数orderItemData的要求[{'cartId':'','gameId':'','price':'','quantity':''}]
    ...
    window.order_submit_popup1 = function (orderItemData) {
        var form_str = '<form class="layui-form" lay-filter="orderVal" style="margin: 20px"> '
            + '<div class="layui-form-item"> '
            + '<label class="layui-form-label">报修人</label> '
            + ' <div class="layui-input-block"> '
            + '<input type="text" name="consigneeName" required  lay-verify="required" placeholder="请输入报修人姓名" autocomplete="off" class="layui-input"></div> '
            + '</div><div class="layui-form-item layui-form-text"> '
            + '<label class="layui-form-label">报修地址</label><div class="layui-input-block"> '
            + '<textarea name="address" placeholder="请输入报修地址" class="layui-textarea"></textarea>'
            + '</div></div><div class="layui-form-item"> '
            + '<label class="layui-form-label">报修事由</label><div class="layui-input-block"> '
            + '<textarea name="zip" placeholder="请输入报修事由" class="layui-textarea"></textarea> '
            + '</div></div><div class="layui-form-item"> '
            + '<label class="layui-form-label">手机号码</label><div class="layui-input-block"> '
            + '<input type="text" name="phoneNumber" required  lay-verify="required" placeholder="请输入手机号码" autocomplete="off" class="layui-input"> '
            + '</div></div>';
