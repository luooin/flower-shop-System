layui.use(['element', 'jquery', 'layer', 'laytpl','laypage'], function() {
	...
	$.getJSON("/index/category",function(res){
		...
		//请求全部商品
		getGamesByPage('/index/games',{page:1,limit:10});
	});
	...
	element.on('tab(categoryTabBrief)', function(data) {
		...
		if(!($(this).attr('lay-id')==='search')){
			...
			getGamesByPage('/index/games',{page:1,limit:10,categoryCode:code});
		}else {
			$("#content").html('');
		}
	});
	...
	$("#search-btn").click(function(){
		...
		let gameName=$("#keyword-input").val().trim();
		if(gameName.length==0){
			return;
		}
		getGamesByPage('/index/games/search',{page:1,limit:10,gameName:gameName});
	});
	 ...
	 * @param param
	 */
	function getGamesByPage(url,param) {
		...
		$.getJSON(url,param,function (result) {
			...
			laytpl($("#game-card-tpl").html()).render(result.data,function(html){
				$("#content").html(html);
			});
			//调用分页
			laypage.render({
